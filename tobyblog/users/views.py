...
def register(request):
    if request.method == "POST": 
        ...
        if form.is_valid():
            ...
            messages.success(request, "Successfully Screated account for: {}. You'll now be able to Login.".format(username) )
...
def profile(request):
    if request.method == "POST":
        ...
        if u_form.is_valid() and p_form.is_valid():
            u_form.save()
            p_form.save()
            messages.success(request, 'Account update has been successful.')
